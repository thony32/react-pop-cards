...
function buildCodeString(disposition: Disposition, bgColor: string, isRounded: boolean, tension: number, friction: number): string {
    const parts = [`disposition="${disposition}"`, `tension={${tension}}`, `friction={${friction}}`, `isRounded={${isRounded}}`, `bgColor="${bgColor}"`]
    return `<Card data={array} ${parts.join(' ')} />`
}
...
export function Sandbox() {
    ...
    // Sync hex color from color picker
    useEffect(() => {
        setBgColor(hexColor)
        if (bgColor !== color) {
            setTextColor(hexColor)
        }
    }, [hexColor, bgColor, color])

    // Code preview
    const code = useMemo(() => buildCodeString(disposition, bgColor, isRounded, tempTension, tempFriction), [disposition, bgColor, isRounded, tempTension, tempFriction])
        ...
        <>
                        ...
                        <div className="flex justify-center">
                            <div className="flex flex-col justify-center">
                                <div className="flex justify-between items-center mb-2">
                                    <h2 className="text-2xl font-fortnite">Installation :</h2>
                                    <ThemeChanger />
                                </div>
                ...
                <div className="max-sm:flex max-sm:flex-col max-sm:gap-16 xl:grid xl:grid-cols-11 gap-[5%]">
                    ...
                    <div className="scrollbar xl:col-span-3 space-y-10 px-5 py-3 overflow-y-scroll h-[70vh]">
                        ...
                        <div className="drawer">
                            <input id="data-drawer" type="checkbox" className="drawer-toggle" />
                            <div className="drawer-content">
                                <div className="flex flex-col gap-3 max-sm:items-center items-start">
                                    <h2 className="text-2xl font-fortnite">Data (must be set) :</h2>
                                    <label htmlFor="data-drawer" className="btn btn-info drawer-button max-sm:w-full">
                                        Set
                                    </label>
                                </div>
                            </div>
                            <div className="drawer-side z-50">
                                <label htmlFor="data-drawer" className="drawer-overlay" />
                                <div className="bg-base-100 h-screen p-2">
                                    <h3 className="mb-1 sm:text-xl uppercase font-fortnite">Set data here like the example here :</h3>
                                    <Editor
                                        height="50vh"
                                        width={isMobile ? '30vw' : '50vh'}
                                    ...
                                    <div className="mt-2 space-y-4">
                                        ...
                                        <div className="flex justify-end">
                                            <button onClick={handleApplyData} className="btn btn-info">
                                                Set data
                                            </button>
                                        </div>
                                    </div>
                        ...
                        {/* Color picker */}
                        <div className="space-y-3">
                            <h2 className="text-2xl font-fortnite">Color :</h2>
                            <div className="flex justify-center">
                                <ColorPicker
                                    value={color}
                                    hideAdvancedSliders
                        ...
                        {/* Disposition */}
                        <div className="space-y-3">
                            <h2 className="text-2xl font-fortnite">Disposition (default : LeftRight)</h2>
                            <div className="grid grid-cols-2 gap-3">
                                {(
                                    [
                                        { value: 'LeftRight', label: 'Left to right', hideOnMobile: true },
                                        { value: 'RightLeft', label: 'Right to left', hideOnMobile: true },
                                        { value: 'TopBottom', label: 'Top to bottom', hideOnMobile: false },
                                        { value: 'BottomTop', label: 'Bottom to top', hideOnMobile: false }
                                    ] as const
                                ).map(({ value, label, hideOnMobile }) => (
                                    <div key={value} className={hideOnMobile && isMobile ? 'hidden' : 'form-control'}>
                            ...
                            <div className="text-xs text-justify">
                                But for mobile devices &ldquo;LeftRight&rdquo; is set as &ldquo;TopBottom&rdquo; and &ldquo;RightLeft&rdquo; is set as &ldquo;BottomTop&rdquo;
                            </div>
                        </div>

                        {/* Radius */}
                        <div className="space-y-3">
                            <h2 className="text-2xl font-fortnite">Radius (default : false)</h2>
                            <div className="form-control">
                                <label className="cursor-pointer label space-x-5">
                                    <span className="label-text">Is Rounded</span>
                                    <input type="checkbox" checked={isRounded} onChange={() => setIsRounded((prev) => !prev)} className="checkbox checkbox-info" />
                                </label>
                            </div>
                        </div>
                        ...
                        {/* Tension & Friction */}
                        <div className="space-y-3">
                            <h2 className="text-2xl font-fortnite">Tension and friction (default tension: 120, default friction: 10)</h2>
                            <div className="flex max-sm:flex-col max-sm:items-start items-end gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setTempTension(Number(e.target.value))}
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="10"
                                        className="input input-bordered w-full max-w-xs"
                                        onChange={(e) => setTempFriction(Number(e.target.value))}
                                    />
                                </div>
                                <button className="btn btn-info max-sm:w-full" onClick={applyTensionFriction}>
                                    Set
                                </button>
                            </div>
                        </div>
                    ...
                    <div className="xl:col-span-8 flex flex-col justify-between space-y-4">
                        <div>
                            <Card data={cardData} bgColor={bgColor} disposition={disposition} isRounded={isRounded} tension={tension} friction={friction} />
                        </div>

                        <div className="w-full flex justify-center">
                            <div>
                                <h2 className="text-2xl font-fortnite mb-1">Code preview</h2>
                                <div className="mockup-code relative">
                                    <pre>
                                        <code id="code" className="whitespace-pre-wrap">
                                            {code}
                                        </code>
                                        <button className="absolute top-2 right-2 rounded-full bg-base-200 active:scale-90 duration-300" onClick={copyToClipboard} aria-label="Copy code">
                                            <svg className="w-7 h-7 p-1" viewBox="0 0 24 24" fill="none">
                                                <path
            ...
            <footer className="w-full py-3 px-[7%] bg-base-200/50 grid grid-cols-3">
                <div className="flex gap-3 items-center justify-start">
                    <a className="hover:scale-110 duration-150" href="https://github.com/thony32/react-pop-cards" target="_blank" rel="noreferrer" aria-label="GitHub">
                    ...
                    <a className="hover:scale-110 duration-150" href="https://www.linkedin.com/in/anthony-mahefasoa-3672361b2/" target="_blank" rel="noreferrer" aria-label="LinkedIn">
